...
import {
	DeleteGuestParams,
	RegisterNewBody,
	UpdateGuestCallingStatusBody,
	UpdateGuestCallingStatusParams,
...
/* --------------------------------- ANCHOR create --------------------------------- */
export const registerNewGuestHandler = async (
	...
	try {
		...
		const user = await GuestModel.create({ name, email, phone });

		const token = generateJWT({ id: user._id }, "24h");

		httpOnlyCookie("guestToken", token, res);
...
export const getAllGuestsHandler = async (req: Request, res: Response) => {
	try {
...
export const updateGuestCallingStatusHandler = async (
	req: Request<UpdateGuestCallingStatusParams, {}, UpdateGuestCallingStatusBody>,
	res: Response,
) => {
	try {
		const { id } = req.params;
...
export const deleteGuestHandler = async (
	...
	try {
		...
		const deletedUser = await GuestModel.findByIdAndDelete(id);
...
/* ------------------------------- ANCHOR verify user ------------------------------ */
export const verifyGuestHandler = (req: Request, res: Response) => {
	try {
		...
		const verifiedUser = verifyJWT(guestToken);
